...
> status: **confirmed**（2026-08-01 PO 承認 — receipt 66b03428ea58）。JSON 内容正本の生成ビュー（全層再降下 §5）
...
| TCC-23-2 | reject | AC-23-2 | — | kpi_nodes 空のまま | operation_log +2 行（指標拒否・URL 拒否） | 構造化ログの拒否行（指標名・URL・理由） | kpi_nodes への行追加・外部への実遷移 | 0 回 | S0 |
...
| TCC-32-1 | normal | AC-32-1 | — | draft 1 件（全値出典つき・未昇格の draft 状態） | draft +1 件、evidence +1 行（operation_log — Web 取得） | draft の出典 URL 列＋Web 取得の外部操作証跡（evidence kind = operation_log） | draft の正本への自動昇格・外部への書込み | 0 回 | S1 |
...
| TCC-41-2 | reject | AC-41-2 | — | 経路は 1 件も返却されない | operation_log +2 行（未登録拒否・有償経路拒否） | 構造化ログの拒否行（service・要求経路・理由） | 有償 API への接続試行（外部 HTTP 呼出 0 回）・spend_ledger への行追加 | 0 回 | S0 |
| TCC-41-3 | boundary | AC-41-3 | — | 両要求とも経路返却 0 件 | operation_log +2 行（破損拒否・経路なし） | 構造化ログの拒否行（理由 = registry 行破損／fallback なし） | 破損行からの部分的な経路返却（fail-open）・外部 HTTP 呼出（0 回） | 0 回 | S0 |
...
| TCC-43-1 | normal | AC-43-1 | — | playbook = active（新地図） | playbooks 1 行 UPDATE、operation_log +2 行（検知・再生成成功） | 秘匿化済み構造化ログ（不一致セレクタ・再生成結果）＋再解析時 screenshot evidence | 再解析中の外部サイトへの書込み（mock 書込み受信 0 回）・2 回目の再生成試行 | 0 回 | S2 |
| TCC-43-2 | reject | AC-43-2 | — | playbook = broken、task = escalated | playbooks.status UPDATE（broken）、state_transitions +1 行（escalate）、operation_log +2 行（検知・再生成失敗） | 秘匿化済み構造化ログ（再生成失敗理由）＋escalate 遷移行 | 2 回目以降の自動再生成試行・broken 地図での書込み操作続行・地図の推測書換え | 0 回 | S2 |
| TCC-43-3 | boundary | AC-43-3 | — | note 行 = active、kdp 行 = retired のまま対象タスク escalated | playbooks 1 行 UPDATE（active）、state_transitions +1 行（retired 側 escalate）、operation_log +2 行以上 | 秘匿化済み構造化ログ（再開後の再生成試行・retired 拒否） | 中間状態の地図（部分更新）での操作再開・retired 行の自動復活・外部書込み | 0 回 | S2 |
...
| TCC-44-2 | reject | AC-44-2 | — | external_operations に prepared 行すら作られない（検証は送信前） | operation_log +2 行（ペアなし拒否・本番書込み拒否）。external_operations 差分なし | 構造化ログの拒否行（理由 = pair 未成立／非 Docker endpoint） | 外部 HTTP 呼出（受信カウンタ 0 のまま）・本番 WP への一切の書込み | 0 回 | S1 |
...
| TCC-45-2 | reject | AC-45-2 | — | 同期 task = failed、loop_run は継続進行 | operation_log +1 行（NotionUnavailable）、state_transitions に同期 task の failed 遷移＋他 task の正常遷移 | 秘匿化済み構造化拒否ログ（service=notion・理由 = unavailable） | ループ本体の停止・待機（Notion 障害の波及）・障害中の書戻し再送連打 | 0 回 | S1 |
...
| TCC-46-2 | reject | AC-46-2 | — | 公開 0 件、rejected 側 task = failed（retry_count 増加なし） | operation_log +1 行（binding 不一致）、approvals 1 行 UPDATE（rejected）、state_transitions +1 行（failed） | 構造化ログの拒否行（不一致項目の明示）＋approvals rejected 行 | 不一致のままの公開（外部書込み 0 回）・rejected タスクの自動リトライ・escalated への迂回 | 0 回 | S0 |
...
| TCC-47-2 | reject | AC-47-2 | — | 接続 0 件、ログには伏字（token=***）のみ、書出し元 task = escalated 誘導 | 外部操作開始前のため operation_log 差分なし。秘匿化済み構造化検知ログと task の escalate 遷移だけを記録 | 秘匿化済み構造化検知ログ（秘匿値そのものを含まない） | 平文のままのログ永続化・秘匿値なしでの外部接続試行（外部 HTTP 呼出 0 回） | 0 回 | S0 |
| TCC-47-3 | boundary | AC-47-3 | — | 再投入前 = escalated・接続 0 件、再投入後 = 同一タスクが接続成功 | operation_log +2 行（期限切れ・組合せ拒否）、state_transitions に escalate と再開の遷移 | 秘匿化済み構造化拒否ログ（理由 = session expired／credential-endpoint mismatch — 平文なし） | 期限切れセッションでの外部送信・テスト credential の本番 endpoint 使用（fail-open）・再投入値の SQLite 保存 | 0 回 | S0 |
...
| TCC-51-2 | reject | AC-51-2 | — | assets 空のまま、WP モックへの書込み呼出 0 回 | operation_log +2 行（未 commit 拒否・接続先拒否）のみ | 構造化ログの拒否行（理由 = uncommitted source／wp target denied） | assets/evidence への行追加・WP（モック含む）への書込み呼出 | 0 回 | S0 |
...
| TCC-52-2 | reject | AC-52-2 | — | レンダリング出力 0 件 | operation_log +1 行（取得失敗・キャッシュなし拒否）のみ | 構造化ログの拒否行（理由 = token unavailable, no cache） | トークン未適用出力の生成・assets/evidence への登録 | 0 回 | S1 |
...
| TCC-53-2 | reject | AC-53-2 | — | assets 差分なし、WP への登録 0 件 | operation_log +2 行（参照拒否・実行失敗）のみ | 構造化ログの拒否行（理由 = missing asset ref／pipeline failed at encode） | 部分出力（中間 mp4）の assets 登録・WP アップロード | 0 回 | S3+ |
...
| TCC-54-2 | reject | AC-54-2 | — | review_pass 証跡 0 件のまま | operation_log +1 行（hash 不一致拒否）のみ | 構造化ログの拒否行（理由 = commit hash mismatch H1≠H2） | H2 での review_pass 記録・既存 commit_hash 証跡の書換え | 0 回 | S0 |
...
| TCC-55-2 | reject | AC-55-2 | — | assets 差分なし | operation_log +2 行（実体混入拒否・参照不正拒否）のみ | 構造化ログの拒否行（理由 = content body in metadata／parent not found） | 本文実体を含む行の INSERT・出自なし派生行の作成 | 0 回 | S0 |
| TCC-55-3 | boundary | AC-55-3 | — | assets 1 行のまま（A1）、循環系譜 0 件 | assets 差分なし、operation_log +1 行（循環拒否） | 構造化ログの拒否行（理由 = circular lineage） | canonical_url 重複行の作成・循環系譜の成立 | 0 回 | S0 |
...
| TCC-61-2 | reject | AC-61-2 | — | kpi_nodes は seed の行のみ | operation_log +3 行（有料拒否・重複拒否・越境拒否）のみ | 構造化ログの拒否行（指標型・node_key・理由） | cac/roas/ad_spend 型ノードの成立（アプリ層を迂回した直接 INSERT も DDL CHECK で拒否されること） | 0 回 | S0 |
...
| TCC-62-2 | reject | AC-62-2 | — | 部分破損: measurements 7 行＋隔離 3 行。全破損: measurements 差分なし | 部分: evidence +1・measurements +7・operation_log +1（隔離記録）。全破損: operation_log +1 のみ | 秘匿化済み構造化ログ（隔離件数・理由）／取得証跡（部分破損側は投入前に記録済み） | 破損行の measurements 混入・全破損ファイルからの部分コミット | 0 回 | S0 |
...
| TCC-SR-01-2 | reject | AC-SR-01-2 | — | 両 run の状態・学習正本とも提出前のまま | operation_log +2 行（越境拒否 ×2）のみ | 構造化ログの拒否行（loop_kind・提出型・理由） | 越境成果物の永続化・単一ループへの統合的書込み | 0 回 | S1 |
...
| TCC-SR-02-2 | reject | AC-SR-02-2 | — | observation 未受理 | 構造化ログ +1 行（拒否）のみ | 構造化ログの拒否行（違反フィールド・理由） | 混在 payload の部分受理・fact の自動書換え | 0 回 | S1 |
| TCC-SR-02-3 | boundary | AC-SR-02-3 | — | 観測 0 件受理、TLP は正規経路で受理 | operation_log +1 行（判定不能拒否）、tactical_learning_packets +1 行 | 構造化ログの拒否行（理由 = schema 判定不能） | 判定不能時の受理（fail-open） | 0 回 | S1 |
...
| TCC-SR-03-2 | reject | AC-SR-03-2 | — | モデル 0 件受理 | 構造化ログ +2 行（拒否）のみ | 構造化ログの拒否行（欠落フィールド一覧） | 部分受理・自由 JSON の正本混入 | 0 回 | S1 |
...
| TCC-SR-04-2 | reject | AC-SR-04-2 | — | segment 0 件受理 | 構造化ログ +1 行（拒否）のみ | 構造化ログの拒否行（欠落状況フィールド一覧） | ペルソナ型 segment の正本混入 | 0 回 | S1 |
...
| TCC-SR-05-2 | reject | AC-SR-05-2 | — | 0 件受理 | 構造化ログ +2 行（拒否）のみ | 構造化ログの拒否行（欠落要素一覧） | 反証不能な仮説の正本混入 | 0 回 | S1 |
...
| TCC-SR-06-1 | reject | AC-SR-06-1 | — | strategic_briefs 空のまま | 構造化ログ +2 行（拒否）のみ | 構造化ログの拒否行（欠落・無効理由） | 無効 brief の INSERT・digest の発番 | 0 回 | S0 |
...
| TCC-SR-10-2 | reject | AC-SR-10-2 | — | v1 = active のまま・revision accepted 0 件 | 構造化ログ +2 行（拒否）のみ | 構造化ログの拒否行（根拠不足／反証未明示） | 単一計測値による自動 accept・新版の先行生成 | 0 回 | S1 |
...
| TCC-SR-12-2 | reject | AC-SR-12-2 | — | 意味モデル不変（v1 = active） | 構造化ログ +1 行（拒否）のみ | 構造化ログの拒否行（理由 = 単一計測値根拠） | KPI 変動による戦略正本の自動更新 | 0 回 | S1 |
...
| TCC-SR-13-2 | reject | AC-SR-13-2 | — | 企画未承認・plan_record 0 件 | 構造化ログ +1 行（拒否）のみ | 構造化ログの拒否行（欠落キー = recognition_change） | 宣言なし企画の主要企画としての承認 | 0 回 | S1 |
...
| TCC-SR-14-2 | reject | AC-SR-14-2 | — | brief 0 件発行 | 構造化ログ +2 行（拒否）のみ | 構造化ログの拒否行（宣言値・台帳版） | 媒体名の media_role としての保存・揺れの自動正規化 | 0 回 | S1 |
...
| TCC-41-4 | reject | AC-41-4 | — | registry 不変（x の browser 書込み行なし） | operation_log に拒否 1 行 | 拒否の構造化ログ（FN-704。状態遷移拒否は state_transitions の拒否行）（BR-M-X-4 理由つき） | x の browser 書込み経路行の混入・後続経路解決での採用 | 0 回 | S0 |
...
| TCC-NFR-01 | reject | AC-901 | NFR-1:invalid-rejection NFR-1:disable-path-absence NFR-1:indeterminate-rejection NFR-1:evidence-channel NFR-1:fatal-escalation | CI/schema拒否は状態不変、未定義遷移は状態不変、ゲート破損はfatal_failureでescalated | CI/schema拒否は差分なし、未定義遷移だけrejected +1、ゲート破損はescalated +1、operation_log差分なし | ゲート失敗ログとstate_transitions拒否行 | 判定不能入力の通過・disable/bypass経路・業務状態更新・operation_log誤記録 | 0 回 | S0 |
...
| TCC-NFR-03 | kill | AC-903 | NFR-3:all-kill-points NFR-3:no-resend NFR-3:evidence-before-transition NFR-3:unknown-escalation | 正準ResumeActionまたはunknownで停止し証跡先行順序を維持 | confirmed/unknownへ一意収束し重複行なし | external_operations遷移とoperation_log照合結果 | 推測完了・再送・メモリ依存再開・証跡より先の業務遷移 | mock WP 受信 0 回（再開時） | S0 |
| TCC-NFR-04 | reject | AC-904 | NFR-4:secret-zero NFR-4:masking NFR-4:runtime-failure NFR-4:credential-rotation-escalation | secret検出0件、違反task escalated、credential rotation escalation発行済み | 正常マスク済み証跡と違反escalate遷移1行だけ、平文差分なし | secret-scan、マスキング、失効・再発行要求の秘匿化済みescalation | credential永続化・違反task継続・rotation要求欠落 | mock 1 回 | S0 |
| TCC-NFR-05 | normal | AC-905 | NFR-5:transition-coverage NFR-5:gate-decision-coverage NFR-5:external-operation-coverage NFR-5:single-query-backlog | 滞留SQLが非終端6状態だけを返す | 読取のみで差分なし | 全state_transitions・pairゲート判定・外部操作対応ログ | 終端混入・複数query結合・遷移/pairゲート/外部操作の証跡欠落 | 0 回 | S0 |
| TCC-NFR-06 | boundary | AC-906 | NFR-6:projected-cap NFR-6:free-route-continuity NFR-6:ledger-completeness NFR-6:config-missing-failclose | projected total超過・config欠落の有償taskはescalated、無償taskは開始可能、台帳漏れ0件 | 拒否時の支出・外部操作差分なし、拒否遷移のみ | state_transitions拒否行、月次集計、external_operation_id台帳対応 | 超過有償操作・config欠落fail-open・無償経路停止・支出欠落/二重計上 | 0 回 | S0 |
| TCC-NFR-07 | boundary | AC-907 | NFR-7:interval-range NFR-7:uniformity NFR-7:daily-cap NFR-7:seed-recording NFR-7:cap-wait-state | 10,000標本χ² p>=0.01、note上限到達後task pending・親loop_run waiting | 許可分だけ外部操作追加、拒否分差分なし | 秘匿化済み実行ログのseed・標本集計・χ²統計量/p値・上限判定 | 範囲外/非一様/固定間隔・seed欠落・note2件超過・拒否後呼出 | 上限到達後 0 回 | S0 |
...
- **TCC-NFR-01**: `NFR-1:invalid-rejection` → invalid fixture全件が非0終了することをassert ／ `NFR-1:disable-path-absence` → src/config検索でdisable・bypass経路0件をassert ／ `NFR-1:indeterminate-rejection` → 破損schemaと判定不能入力が全件拒否されることをassert ／ `NFR-1:evidence-channel` → 経路別にDB差分なし又はstate_transitions拒否行をassert ／ `NFR-1:fatal-escalation` → gate破損時にtask escalatedと遷移1行をassert
...
- **TCC-NFR-05**: `NFR-5:transition-coverage` → 全許可・拒否遷移fixtureを発火しstate_transitions対応行をassert ／ `NFR-5:gate-decision-coverage` → pair_plan_qualityとpair_kpi_measureの成立・拒否fixtureを発火し判定行をassert ／ `NFR-5:external-operation-coverage` → 全external_operations行に対応operation_logが一意にあることをassert ／ `NFR-5:single-query-backlog` → UNION ALL 1 statementの結果集合が全非終端seedと一致をassert
- **TCC-NFR-06**: `NFR-6:projected-cap` → 既支出+要求額の直前・一致・1円超過境界をassert ／ `NFR-6:free-route-continuity` → 有償拒否時も無償task開始可能をassert ／ `NFR-6:ledger-completeness` → confirmed有償operationとledgerの双方向一意対応をassert ／ `NFR-6:config-missing-failclose` → cap欠落時に有償外部呼出0とtask escalatedをassert
- **TCC-NFR-07**: `NFR-7:interval-range` → 4 seed各10000標本が両端包含min/max内100%をassert ／ `NFR-7:uniformity` → MT19937 randint整数bucketで各seed χ² p値0.01以上をassert ／ `NFR-7:daily-cap` → note confirmed 1/2件境界で2件目許可・3件目拒否をassert ／ `NFR-7:seed-recording` → seed・algorithm ID記録と再生列完全一致をassert ／ `NFR-7:cap-wait-state` → 上限拒否後task pending・親loop_run waiting・外部呼出0をassert
