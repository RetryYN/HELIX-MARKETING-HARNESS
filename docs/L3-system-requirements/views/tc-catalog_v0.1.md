...
| TCC-42-2 | reject | AC-42-2 | external_operations に新規 sent/confirmed 行なし | operation_log +2 行（X 拒否・上限拒否）。external_operations 差分なし | 構造化ログの拒否行（media・理由 = prohibited／daily_cap） | 外部サイトへのブラウザ書込み送信（0 回）・playbooks.last_success_at の更新 | 0 回 | S1 |
