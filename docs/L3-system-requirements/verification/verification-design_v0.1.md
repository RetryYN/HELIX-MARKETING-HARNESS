---
artifact_id: L3-VERIFICATION-DESIGN
...
> TC 検証契約 = [json/verification/tc-contracts.json](tc-contracts.json)
> （ビュー [tc-catalog_v0.1.md](../views/tc-catalog_v0.1.md)）。旧体系の TC 台帳は historical 記録のみとし、
> AC↔TC の双方向接続は G-TRACE-BIDIR が fail-close 検査する。
> 上流戦略層（2026-08-01 追補）: 戦略層要件（SR）の検証は
> [strategy-loop-test-design_v0.1.md](../../L4-basic-design/integration-tests/strategy-loop-test-design_v0.1.md)（STC）が対を成す。
> 本書が定める検証の対象は不変（現行分母は AC/TC 契約正本 = baseline.json の contract_counts）。
...
| [media-requirements_v0.1.md](../canonical/functional/media-requirements_v0.1.md) | §2、§3、§6 | WP と MEAS の S0 契約。その他媒体は deferred |

逆方向では、各 TC の `ac`、`text`、fixture、更新境界を JSON 台帳から設計文書へ追跡する。TC の追加・削除時は、設計側の対象節又は N/A を必ずペア台帳へ反映する。

## 2. AC→テストケース設計

fixture はすべて [json/s0/environment.json](../canonical/schemas/s0/environment.json) に従う。`sqlite_seed` は一時 SQLite と seed を使う unit/integration fixture、`wp_docker` は唯一の実書込み先であるローカル Docker WP、`ga4_mock` は GA4 fixture/mock、`approval_mock` は通知 transport mock を表す。本番 WP はいずれの TC も書き込まない。
...
- JSON 正本は `python3 -m json.tool docs/L3-system-requirements/verification/tc-contracts.json` で検証する。
- Markdown は MD013 を無効化した markdownlint で 0 issue を完了条件とする。
- AC 検証契約の全 ID が `tc-contracts.json` の少なくとも一つの `items[].ac` に存在することを G-TRACE-BIDIR が機械照合する。
