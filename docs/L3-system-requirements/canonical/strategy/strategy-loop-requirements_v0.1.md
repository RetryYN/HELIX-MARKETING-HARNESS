...
> （ビュー [sr-contracts_v0.1.md](../../views/sr-contracts_v0.1.md)。G-FRSR-CONTRACT が fail-close 検査）。
>
...
- **SR-14 媒体役割語彙**: brief の `media_role` は媒体名ではなく戦略上の役割
  ...
  （[json/strategy/media-roles.json](media-roles.json)）を正本とする。
...
- **SR-13 認識変化資産**: コンテンツを投稿物・集客物としてだけ扱わない。コンテンツ＝市場の問題認識・
  ...
  （[json/strategy/content-plan-contract.json](content-plan-contract.json) が正本）。

## 6. S0 との境界（スコープ拡大禁止）

S0 実装分の SR には受入条件 **AC-SR-01〜06**（[json/strategy/ac-sr.json](ac-sr.json) が
...
（台帳: [requirements-gates.md](../../../00-authority/requirements-gates.md)。各ゲートは
