...
> 各 NFR の計測契約 = [json/nfr/nfr-contracts.json](../nonfunctional/nfr-contracts.json)
> （ビュー [nfr-contracts_v0.1.md](../../views/nfr-contracts_v0.1.md)）、AC 検証契約 =
> [json/ac/ac-contracts.json](../acceptance/ac-contracts.json)（ビュー [ac-catalog_v0.1.md](../../views/ac-catalog_v0.1.md)）。
...
- **FR-31** ヒアリングエンジン: スキーマ（§4）の必須スロットの空きを検出し、質問リストを生成して
  構造化回答 UI/API で人に照会、回答を型検証して充填する。Claude Code は任意クライアントであり、未充足のままでは依存タスクを開始しない
...
- **FR-46** 承認チャネル: VPS 側承認 API を正本とする交換可能 transport（初期 Discord、将来 Web UI / PWA）で束縛承認（対象・操作・時点を明記）を送り、
...
- **FR-62** 取り込みパイプライン: 取得（正規 API またはブラウザエクスポート。経路は接続レジストリと
  [ADR-006](../../../00-authority/adr/ADR-006-official-api-routes.md) に従い、GA4/GSC は正規 API）→
...
JSON 正本（[ac-contracts.json](../acceptance/ac-contracts.json)）に持ち、
...
[strategy-loop-requirements_v0.1.md](../strategy/strategy-loop-requirements_v0.1.md)（SR-01〜16）と
[strategy-learning-contract_v0.1.md](../strategy/strategy-learning-contract_v0.1.md) を正本とする。
...
- 承認: 文書の draft → confirmed 昇格は [approvals.md](../../../00-authority/approvals/approvals.md) に記録する
- 用語: 独自語の定義は [glossary_v0.1.md](../../../L1-business-requirements/canonical/glossary_v0.1.md) を正本とする
