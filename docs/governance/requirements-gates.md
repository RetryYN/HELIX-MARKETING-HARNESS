...
| G-DDL-APPLY | DDL が空 SQLite へ適用でき FK/integrity 検査が通り 25 テーブル＋append-only トリガ 10 | 実行不能なスキーマ |
...
| G-OBS-INTERPRETATION | market_observation は解釈フィールドを持てず（additionalProperties: false）、解釈は TLP の分離フィールドのみ。混在 fixture を拒否 | 観測事実と AI 解釈の混濁（反証不能化） |
| G-LEARNING-TRACE | 全 tactical_learning_packet が loop run・brief digest・evidence へ接続。未接続 fixture を拒否 | 接続のない学習（存在しない扱いにすべき還流） |
...
| G-REVISION-EVIDENCE | strategy_revision に根拠・反証・信頼度・対象版が必須。支持根拠 2 件未満の accepted（単一計測値の自動 accept）を拒否 | KPI 値の直接戦略変換（意味モデルを迂回した自動変更） |
...
| G-STRAT-PAIR | 戦略層 4 文書（要件・契約・設計・テスト設計）が相互 pair 参照を持ち、SR 16／SCM 10 が STC で双方向カバーされ、全戦略ゲートに拒否系 STC（negative test）が存在 | 片肺の戦略層（検証なき意味モデル） |
