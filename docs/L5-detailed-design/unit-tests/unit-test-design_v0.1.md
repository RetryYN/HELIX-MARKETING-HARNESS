...
> pair: [detailed-design_v0.1.md](../canonical/detailed-design_v0.1.md)（詳細設計⑤ — HELIX 式 ⑤↔⑥ 文書ペア）
> 対象文書: detailed-design_v0.1.md の DU-01〜DU-23 全モジュール。
> 上位文書: [verification-design_v0.1.md](../../L3-system-requirements/verification/verification-design_v0.1.md)（③ — TC 検証契約の正本。
> 本書は TC を再定義せず、**各 TC を実装先 DU へ割当てる**）
> JSON 正本: [du-contracts.json](../canonical/apis/du-contracts.json) の `trace.tc` / `apis[].ut`
> （割当と API 単位 UT の正本。旧 UT 台帳は historical 記録のみ）

---

## 1. 位置づけと合否基準

- 本書は ③の TC 検証契約を **どの DU の単体テストとして pytest 化するか**を 1 対 1 で確定し、
...
- 本台帳は分母ラチェット対象（DU 23・API 単位 UT は du-contracts の `apis[].ut` が正本）。縮小・割当解除は
