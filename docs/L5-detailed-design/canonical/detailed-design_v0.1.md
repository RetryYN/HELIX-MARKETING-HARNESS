---
artifact_id: L5-DETAILED-DESIGN
...
> [s0-contract_v0.1.md](../../L3-system-requirements/canonical/s0-contract_v0.1.md)（DDL・遷移・evidence 型・WF 契約の正準）
> JSON 正本: [json/detailed.json](modules/detailed.json)（DU 台帳＝分解・対応の正本。**公開 API 署名の正本は本書 MD**）
...
> 競合制御・AC/TC/UT 対応 = [json/du-contracts.json](apis/du-contracts.json)
> （ビュー [du-contracts_v0.1.md](../views/du-contracts_v0.1.md)）。重要機能の機能別詳細設計 = [features/](../../L6-feature-design/S0/)
...
- `transition(conn, entity_type: str, entity_id: int, event: str, actor_agent_id: int | None, details: dict, clock: Clock) -> TransitionResult`
  — 遷移表（docs/L3-system-requirements/canonical/schemas/s0/transitions.json をパッケージデータとして同梱）を照合。
