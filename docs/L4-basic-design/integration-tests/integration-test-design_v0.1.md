...
> pair: [basic-design_v0.1.md](../canonical/basic-design_v0.1.md)（基本設計② — HELIX 式 ②↔④ 文書ペア）
> 対象文書: basic-design_v0.1.md の CMP-01〜CMP-13 全コンポーネント。
> 上位文書: [verification-design_v0.1.md](../../L3-system-requirements/verification/verification-design_v0.1.md)（検証設計③ — TC 検証契約は
> ユニット/コンポーネント粒度、本書 ITC はコンポーネント結合〜E2E 粒度で階層を分担する）
> JSON 正本: [json/itest.json](itest.json)（ITC 台帳。本文と同期、実装入力は JSON）
...
- ③（TC 検証契約）が「各 AC を最小粒度で示す」のに対し、④（ITC 16）は「CMP を結合した実シナリオで
