...
> 独立設計書: [外部 IF](external-if/external-if-design_v0.1.md)／[DB](data/db-design_v0.1.md)／
> [状態機械](state-machine/state-machine-design_v0.1.md)／[エラー分類](../../L5-detailed-design/canonical/errors/error-taxonomy_v0.1.md)／
> [承認](approval/approval-design_v0.1.md)／[ブランド隔離](brand-isolation/brand-isolation-design_v0.1.md)
> （G-CMP-INTERFACE が fail-close 検査）。
> JSON 正本: [json/components.json](components/components.json)（コンポーネント台帳。本文と同期、実装入力は JSON）
> 位置づけ: S0（25 機能）の実装構造を確定する。要求内容は定義しない — 上位文書と矛盾したら上位を優先し本書を改訂する。
> 上流戦略層（2026-08-01 追補）: 戦略コンポーネント（SCM）は
> [strategy-loop-design_v0.1.md](components/strategy-loop-design_v0.1.md) が正本。S0 分は CMP-05（テーブル・トリガ）・
...
1. **契約駆動**: DB スキーマ・状態遷移・evidence 型・WF 実行は [s0-contract](../../L3-system-requirements/canonical/s0-contract_v0.1.md) が正準。
...
tests/
  unit/        # ③TC 検証契約のうち unit/component 粒度を pytest 化
...
（ゲート G-CMP-FN が機械検証）。各 CMP の正本レコードは [json/components.json](components/components.json)。
...
- 遷移表（docs/L3-system-requirements/canonical/schemas/s0/transitions.json（リポジトリ相対）をビルド時に埋め込み）を照合し、未定義の (entity, from, event) は
