...
# 機能設計: ブランド隔離 S1 完成（スコープ型・強制実装・複数ブランド・越境 negative test）
...
> 対スライス文書: [brand-isolation-foundation.md](../S0/brand-isolation-foundation.md)（S0 = schema 共存・単一ブランド運転・名前空間規約のみ）
> 位置づけ: S0 基盤の上に、FR-34 の**強制**を積む実装計画。スコープ型（`ScopeContext`）・解決関数
> （`resolve_scope`）・越境例外（`CrossProfileAccessDenied`）は S0 に存在しないため、**本書が定義の正本**である。
...
S0 では型も解決関数も持たない（S0 基盤文書 §1「S0 で採用しないもの」）。本スライスで新設する。

| 実装単位 | 所属 | 責務 | 失敗方針 |
|---|---|---|---|
| `ScopeContext`（値オブジェクト） | `db/scope.py`（新設・CMP-05 副層） | `business_profile_id`＋`profile_key`＋`mode`（read_write／read_only）の frozen 保持。生成は `resolve_scope` のみ | 生成経路外のインスタンス化を拒否（ゲート通過の型強制と同型） |
...
| 非帰属テーブル | agents・workflows・config 等（上位設計 §1 の共有基盤）はスコープ引数を取らない。config は S0 から使う `<profile_key>.` キー接頭の名前空間を継承する |
...
1. **型と解決経路（§1）**: `ScopeContext`・`resolve_scope`・例外 3 種の新設。ここが無いと
   §2 以降のシグネチャ規約が書けないため最初に置く。
2. **前半（expand）**: `strategic_briefs`・`playbooks` への profile 列 expand ＋ backfill
   ＋新 UNIQUE index（migration 規律 = s0-contract §5）。全ストア API の scope 必須化と
   親チェーン検査の実装。
3. **後半（強制 = FN-306／FR-34）**: §4 の negative test 6 項目を test-first で赤→実装、
   認証・storage_state の物理分離、brief ガードへの profile 一致追加。
4. **完了**: 2 プロファイル目の解禁と、整合性クエリ＋越境テストの LP-OPS 定常実行。
...
| 実装単位 | DU | AC | TCC | 備考 |
|---|---|---|---|---|
| ScopeContext・resolve_scope・例外 3 種 | FN-306 実装 DU（⑤改訂で採番） | AC-34-1 | TCC-34-1 | 生成点の一元化と型強制（S0 には存在しない） |
| ストア API 規約の全面適用 | 同上 | AC-34-1 | TCC-34-1 | プロファイル共存とスコープ付きクエリ |
