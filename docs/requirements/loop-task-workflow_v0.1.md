...
> 位置づけ: [br-media_v0.1.md](br-media_v0.1.md)（BR-M）を実行モデルの 3 構成要素 —
> **ループ（回転）／タスク（作業単位）／ワークフロー（手順）** — に分解した要件定義。
> エンジン要件（FR-11〜16）の媒体別インスタンス定義にあたる。
...
| スライス | 確定実装 | PoC 実施 |
|---|---|---|
| S0 | WF-WP-1/2, WF-MEAS-1, LP-W(WP), T-PLAN/PROD/REVIEW/PUB/MEAS | POC-03（GA4 取得経路）を S0 内で先行 |
| S1 | WF-DASH-1, WF-NOTION-1, LP-MEAS, 還流 | POC-08（音声/動画パイプ）着手 |
| S2 | 三エンジン, LP の複数媒体化 | POC-01, 05, 10 |
| S3+ | 各媒体 WF の本実装（PoC PASS 済みのみ） | POC-02, 09, 11, 12, 13 |
...
- 上流: LP → BR-A2/FR-14、T → BR-A4/FR-12/13/27/28、WF → BR-F3/NFR-8、PoC 規律 → HELIX poc kind
