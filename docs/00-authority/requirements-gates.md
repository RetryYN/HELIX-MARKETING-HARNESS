...
| G-STRAT-PAIR | 旧再検証資料について、SR19／SCM10／AC-SR6 の双方向カバー＋戦略 4 文書の相互 pair＋全戦略ゲートに拒否系 STCが構造上揃うことだけを検査する。現要求の受入権威・実装可否は`G-REQ-STRATEGY-TEST-AUTHORITY`で別にfail-close判定する | 片肺の戦略資料、又は構造カバレッジPASSをPO承認済みの現行受入権威と誤認すること |
...
| G-REQ-LEGACY-MEDIA-TRACE | 通常BR正本とは別台帳にある旧BR-M 70件とMR 54件の隣接traceを双方向に照合し、参照IDの実在とforward/reverse edge一致を要求する | 媒体traceを通常BR traceの圏外に置いたまま、片方向又は幽霊IDの媒体要件を再降下すること |
| G-REQ-LEGACY-MEDIA-TRACE-DISPOSITION | 現行の旧BR-M↔MR fault集合をclass別count＋digestへ分け、全faultが重複なく一度だけ分類されることを検査する。forward／reverse欠落、孤立node、未知ID、空集合を旧edgeの機械補修でなく個別capability revisionへの意味再降下へ送る | fault件数だけを記録すること、分類外又は重複faultを残すこと、旧BR-M/MR proseを現行permissionとして残したまま逆辺だけ追加すること |
...
| G-REQ-PROVIDER-EXECUTION-POLICY | 外部provider経路を公式API→公式MCP→許可CLI adapterの順にし、Playwrightをattended又はread確認へ限定する。consumer Web UI無人操作、個人home証跡、PO束縛のない単一provider必須化を禁止し、provider/account/principal/credential/license/evidenceをtyped exact contractで検査し、そのrevision/digestを全媒体候補とroute refinementへ束縛する | 禁止語を文章に含めるだけで逆方向の許可を通すこと、媒体候補が別route policyを無参照で併存させること、provider・資格情報・ライセンス・証跡を設計者が推測すること |
| G-REQ-LEGACY-CONSUMER-ISOLATION | historical/revalidation扱いの旧REQ／requirements viewを既知consumerから隔離し、manifest上の全implementation inputと非監査codeを導出走査する。製品code及び未許可scriptから旧contract context／gate moduleをimportする経路も拒否する | warningだけを根拠に同一ID・別slice・別traceの旧一覧を現行要求として再利用すること、path分割・定数再export・CTX importで隔離を迂回すること |
| G-REQ-AUTHORITY-NORMALIZATION | 既存REQ55意味inventoryを再利用し、機械ledgerと旧confirmed Markdownの15 ID・19 field差分だけを両source実artifactから再計算する。全rowのPO分類、単一candidate JSON、生成view、trace差分0、manifest/baseline同一commit及びCI-attested独立Goまでcutoverを拒否する | ID件数一致だけでの意味同値化、未分類・複数選択・stale source・手編集view・旧REQ再混入、自己申告digest又は存在しないGo reviewによる実装入力化 |
| G-REQ-NFR-BUSINESS-AUTHORITY-POLICY | 既存NFR11意味inventoryを再利用し、stable business root、actor/principal scope、applicability scope、phase、処遇、owner/resume及びmeasurement authorityだけをtyped overlayへ分離する | 他NFR・旧MR/FR・節番号・risk ID・draft STCによるroot代用、旧phase又は未登録閾値の暗黙昇格、親digest stale又は未分類のままの受入 |
| G-REQ-DESIGN-NOT-STARTED | 要求baselineがrevisingの間、L2〜L6を`revalidation_required`かつ`implementation_input=false`に固定し、L2をdraftに保つ。README／CLAUDE／AGENTS／開発環境契約／要件定義ワークフロー／ADR-012の入口も旧baselineの再検証資料・現行未拘束境界を明示し、L2は旧要求5点書式の評価用draftだけに限定する。旧方式の現在形命令を禁止する | 旧画面・API・DDL・状態・AC/TC・実装単位、又は入口に残る旧方式制約を新VPS UI要求の設計済み成果物・現行実装命令として扱うこと |
| G-REQ-SCOPE-ASSIGNMENT | 旧要求864 IDを`legacy_revalidation_only`に固定し、新refinement全subjectをrequirements governance／initial／follow-on／deferred候補へ一意に割り当てる。VPS UI、初回activation、content gate、risk、research loop、strategy core再降下及びDiscord通知不採用境界はinitial候補、Playwright routeとDiscord communityはfollow-on候補、approval request撤回後にsupersededとなった旧Discord複数用途候補はdeferred履歴へ固定する | 旧Discord・旧phase等を含むIDの初期scopeへの直接昇格、VPS UI/inbox/security/growth/strategy core又はDiscord通知不採用候補の欠落、生成AI/旧媒体の黙示有効化、Discord communityと通知の再混同 |
...
| G-REQ-CANDIDATE-BINDINGS | baseline候補の全PRC見出しを実在refinement meaning ownerへ束縛し、全subjectが少なくとも一つのPRC責務を持つことを検査する。初期通知はVPS UI inbox、Discordはcommunity routeと通知不採用境界を別subjectで所有し、VPS UI判断は毎回の投稿承認でなく初回activation／例外判断であることも固定する | 手書きPRCの孤立、未知subject参照、候補本文とtyped refinementの二重意味化、community routeだけによる通知不採用境界の代替、旧Discord通知又は個別投稿承認の再混入 |
...
| G-REQ-LEGACY-MEANING-INVENTORY | 旧BR 41件・REQ 55件・FR 43件の意味fieldをID別snapshot digestへ固定する。金銭operation承認、観測前因果の捏造禁止、証跡・再開・冪等性、author/verifier分離、secret失効、account lifecycle判断とbinding preflight分離は保持句・禁止句・移送先をexact検査する。残るIDの意味分類が未完の間はcutoverを停止する | group共通文又はID集合だけで固有の価値・安全制約・人間判断を移送済みとみなすこと、原文変更後も古いdigestで通すこと、未分類を完了扱いすること |
| G-REQ-LEGACY-SR-NFR-MEANING-INVENTORY | 旧SR 19件・NFR 11件の原意味をID別snapshot digestへ固定し、value・safety・human judgement・旧方式非継承・meaning owner・deferred再開条件を全30件で検査する。別agentによる戦略採否代替、旧S0固定、draft STCの受入権威化、credential unlock方式の先決め、stable root欠落NFRの黙示採用を拒否し、PO分類receiptまではcutoverを停止する | SR group要約又はNFR value記述だけで個別の判断境界を棚卸し済みとみなすこと、旧phase・table・固定閾値・agent judge・draft testをpositive meaningとして自動継承すること |
| G-REQ-LEGACY-MR-MEANING-INVENTORY | 旧MR 54件のfile/media identityを含む全source意味をID別digestへ固定し、媒体ごとのvalue・safety・human judgement・旧route非継承・meaning owner・再開条件を検査する。API/MCP優先＋必要時Playwright確認、consumer Web UI無人禁止、Discord community限定、LINE API-first、PLAY超後期、X browser write禁止、WP三責務分離、money operation承認を重点検査し、PO分類receiptまではcutoverを停止する | 媒体名又はBR-MだけでMR固有actionsを実行許可にすること、固定provider・browser route・件数・秒数・quota・toolを業務価値として自動継承すること |
| G-REQ-LEGACY-FN-MEANING-INVENTORY | 旧FN 61件のsource snapshotと親semantic row digestをID別に束縛し、FN固有のactor・value・scope・human judgement・side effect、operation owner、evidence、旧phase処置を検査する。親要求又は意味分類が変わればFN分類を失効させ、PO receiptまではcutoverを停止する | 旧slice・provider・browser・Discord・WP・固定toolをcurrent実装へ継承すること、親のないFN-413媒体omnibusを実装許可とすること |
| G-REQ-LEGACY-AC-MEANING-INVENTORY | 旧AC 252件のsource oracleをID別digestへ固定し、親要求・FNのsemantic digest、polarity、AC固有principal/scope/HJ/effect/evidence、旧target_update処置へ束縛する。旧oracle本文をcurrent合格基準へ自動継承せず、PO receiptまではcutoverを停止する | 旧Discord/ApprovalTransport、machine-only auto-mode、SQLite/Docker WP、static HTML/API-only UI、draft strategy STCを受入正本として再利用すること |
| G-REQ-LEGACY-TC-MEANING-INVENTORY | 旧TC 258件の全fieldをsource digestへ固定し、親AC semantic digest、test kind/oracle、critical group、旧phase/alias処置へID別に束縛する。親AC再降下前はTC固有oracle/effectを肯定せず、PO receiptまではcutoverを停止する | 旧fixture/mock/成功結果、Discord/consumer UI/SQLite等の方式、旧TC alias又はdraft STCを現要求の受入証拠として再利用すること |
...
| G-REQ-LEGACY-MEDIA-BR-DISPOSITION | 旧媒体BR 70件を媒体別及びID別source semantic digest＋ID別処置でexact coverageし、各旧意味を対応する媒体policyへdigest束縛する。`replace`又は`defer`、現候補でのfunnel役割、route policy、meaning owner、再開条件、未承認・未設計境界を要求する。媒体IDは一媒体一行。Discord community、affiliate offer authority、LINE API-first、measurement read、provider-neutral design/GenAI、WP責務分離、X browser境界、Stripe顧客金銭operation等を明示する | 媒体名又は旧BRの存在を実行許可とみなすこと、IDの一部意味を媒体共通文で落とすこと、重複media行で検査を逃れること、製品通知とcommunityを混用すること、consumer Web UIやbrowser writeを暗黙に有効化すること、全媒体を一括releaseすること |
...
| G-REQ-LEGACY-PHASE-DISPOSITION | 現行の旧FR→FN／AC／TCC phase不整合及び包含phaseを重複正規化したedge集合としてexact inventory化し、source/target/phaseを型付けした`pending_po_classification`へ隔離する。`phase_typo`、`split_responsibility`、`defer_target`、`redescent_test`のいずれかは`FR-SLICE-AUTHORITY-ALIGNMENT`のPO判断後にのみ確定する | fault件数だけを手書きすること、edgeの入替・追加・脱落又は旧phaseを暗黙採用すること、責務分割をPO決定前に確定済みと扱うこと |
| G-REQ-FR-SLICE-AUTHORITY-ALIGNMENT-POLICY | 旧phase fault 30 edgeをimmutable source snapshotとして保持し、live fault、PO分類、candidate projection、trace再生成、同一commit manifest/baseline及び独立Goを三段階で分離する | raw phase修復時に旧edge履歴を消すこと、S0等の旧phaseを新authorityへ再利用すること、PO receipt又は独立Goなしにcutoverすること |
| G-REQ-TEST-ID-AUTHORITY-ALIGNMENT-POLICY | 旧TC 14 IDのDU参照mappingとAC-SR二重台帳のcurrent ownerを別trackで分類し、TC258意味digest、PO receipt、単一candidate及びcutover証跡へ束縛する | 旧TCの文字列置換、未知TCCへの統合、旧AC台帳のunion又はcurrent owner再利用、PO未分類での実装入力化 |
| G-REQ-L0-NORTH-STAR-AUTHORITY-NORMALIZATION-POLICY | 旧L0 15 clause候補をcharter実section、replacement PRC及び確定PO controlsへ束縛し、PO分類前の新L0正本化を拒否する | 旧VPS/Discord/browser/provider手段の暗黙継承、POD/PRCによるL0の無承認上書き、source/row digest又はscope別supersessionなしのcutover |
| G-REQ-STRATEGY-REQUIREMENT-ADMISSION-POLICY | 旧SR 19件を既存意味inventory、L0 north-star、research/risk/quality候補及び単一test authorityへdigest束縛し、PO分類前のadmissionを拒否する | SR-01〜16の自動初期採用、SR-17〜19の無承認有効化、draft STC又は旧AC-SR unionの再利用、L0/test authority未批准でのcutover |
| G-REQ-AGENT-NEO-HELIX-REDEFINITION-POLICY | AGENT NEO固定SHAについてRDEへ記録済みの12能力分類と旧任意判断7件をcaptured-observation inventoryとして固定し、外部repo read-only・旧成功非流用・authority分離・PO未分類へ束縛する（外部commit実体との照合は分類cutover時の別証拠） | 外部repo write、旧G4/S3/WP成功のadmission化、package/license/SEO/CRM/SNS/API/AIの自動採用、parent digest stale、未承認cutover |
| G-REQ-AGENT-NEO-SITE-BUILD-RELEASE-POLICY | 親でsite_build ownerに分類されたcapabilityだけを候補母集団とし、WP content/platform/security責務、profile authorization、product state及び対象site作用を分離する | 親未分類での先行選択、product-evolution能力混入、AGENT-NEO repo writeと対象site writeの混同、旧G4/WP/別site成功の流用 |
| G-REQ-AGENT-NEO-PRODUCT-EVOLUTION-RELEASE-POLICY | 親でproduct_evolution ownerに分類されたcapabilityだけを候補母集団とし、component互換性・migration・rollback・regression要求と外部repo write権限を分離する | site-build/WP grantのrepo write流用、旧G4/S3/単一site成功の互換性証拠化、要求cutoverによる外部repo write許可 |
| G-REQ-MEDIA-POC-SCRUM-RELEASE-POLICY | 媒体をrelease unit、媒体内operationをcapability incrementとし、WordPress content初回release、Full V標準、PoC非権威、本番write個別grantを分離する | PoC成功の本番権限化、媒体一部成功の全体受入、platform/security混在、未登録媒体/account/operation write |
| G-REQ-FR16-NOTIFICATION-BOUNDARY-POLICY | 安全停止を先に確定し、VPS UI inboxを非権威なoperational state routeとして記録する | 通知先行、通知失敗rollback、seen/ackからresume、Discord・旧ApprovalTransport fallback、旧S0の実装許可化 |
| G-REQ-DISCORD-NOTIFICATION-REJECTION-POLICY | Discordをcommunity marketing専用に隔離し、製品承認・運用・PR通知routeを常にno-sendで拒否する | community credential/evidence共有、別guild fallback、拒否receiptからdecision、旧ApprovalTransport・browser route復帰 |
| G-REQ-VPS-UI-PRIMARY-INTERFACE-POLICY | VPS Web UI＋UI inboxをhuman product entryとし、read・notification state・decision・feedback authorityを分離する | authnだけのwrite、ackからapproval、feedback即時activation、raw credential表示、Discord/CLI/API-only主入口、Playwright判断代替 |
| G-REQ-EXTERNAL-BROWSER-AUTOMATION-ROUTE-POLICY | API/MCP優先、Playwright read確認・個別登録fallback、browser write既定禁止を分離する | browser優先、quota/credential拒否迂回、確認からwrite権威、consumer UI無人操作、別engine暗黙代替、旧成功流用 |
| G-REQ-OFFICIAL-API-ROUTE-AUTHORITY-POLICY | 公式route registryを媒体/account/operation/effect・freshness・terms・quota・credentialへ束縛する | MCP一律優先、未知/stale route、browser暗黙fallback、readからwrite、route成功の恒久grant化 |
| G-REQ-GENAI-EXECUTION-ROUTE-POLICY | 生成routeをprovider-neutral登録へ束縛しconsumer UI無人操作・固定runtime・publish権威化を拒否する | 未登録CLI、consumer Web UI、Codex/Claude必須化、responseからpublish/state変更、quota/credential迂回 |
| G-REQ-AUTOMATED-PUBLISHING-ADMISSION-POLICY | 初回scope activation後のgate合格自動運用、再生成、retry exhaustion時だけのVPS inbox、明示re-activationを検査する | machine activation、failed artifact提示、通常retry通知、blocked rollback、feedback scope拡張、旧auto-mode復帰 |
| G-REQ-CONTENT-QUALITY-GATE-LEARNING-POLICY | content検査、人間確認前再生成、media-account既定scope、外部rule revision、YMYL/brand strictnessを検査する | fail成果物提示、global scope、risk弱化、hard-code rule、quality passのpublish/growth権威化 |
| G-REQ-CONTENT-RISK-CLASSIFICATION-POLICY | claim risk、YMYL、unknown時の最高strictness、source freshness、必要HJをtyped検査する | unknown→low/pass、generic YMYL N/A、preference弱化、AIによるHJ代替、risk passのpublish/state/growth権威化 |
| G-REQ-RESEARCH-LED-CONTENT-GROWTH-POLICY | research→仮説→媒体role→KPI観測→TLP学習とpaid acquisition超後期deferを検査する | researchなし制作、quality/publish成功のgrowth証拠化、媒体横断流用、offer無権限変更、paid前倒し、strategy直接更新 |
| G-REQ-DISCORD-COMMUNITY-MARKETING-ROUTE-POLICY | Discord community marketingだけをBot/guild/channel/operationへ束縛し通知・承認・PR用途と分離する | product通知、approval/PR tuple、self-bot、個人account操作、credential/evidence共有、post成功のgrowth/authority化 |
| G-REQ-VPS-CREDENTIAL-SECURITY-BOUNDARY-POLICY | VPS再起動後の実行停止、有人runtime再初期化とcredential再認可、fresh session/grant、scope exact及びsecret非永続化をtyped検査する | credential単独auto-unlock、旧session/grant再利用、agent自動復帰、raw secret保存、expiry後継続、persistent service暗黙採用 |
| G-REQ-VPS-UI-INBOX-LIFECYCLE-POLICY | inboxをsource stateへの非decision導線へ限定し、source class別create、seen/ack、source由来resolve/expire、terminal後retentionをtyped検査する | seen/ack/reminderのdecision化、独自expiry、active purge、記録失敗rollback、外部fallback、通常retry通知、quality dedupe欠落 |
| G-REQ-WORDPRESS-MAINTENANCE-BOUNDARIES-POLICY | WordPress content/platform/securityの24 operationをowner・effect・child authority・独立release receiptへexact routingする | content grantによる保守、platform receiptによるsecurity解除、security patchによるpublish、operation重複/欠落、旧WP成功の権威化 |
| G-REQ-RESOLVED-SUBJECT-AUTHORITY-COVERAGE-AUDIT | pending=[]の24 recordを実在policy/inventory/gate/fault経路へ一意又は役割分離してexact対応付けする | 未対応record、primary二重化、superseded再混入、authority digest自己追随、legacy meaning/runtime role混同 |
| G-REQ-RATIFICATION-DEPENDENCY-AUDIT | 未批准authorityをsemantic governance、core/foundation、operational SCC、release/compositeへ分け、true-PO pendingとdownstream readinessを分離する | 親飛ばし、SCC部分ready、quality/strategy未決のready化、WordPress偽依存、批准からimplementation/write権威を推論すること |
| G-REQ-LEGACY-TRACE-DISPOSITION | 旧direct BR↔FR/SR、layered BR→REQ→FR/SR/NFR及び意味責務trace faultをcount＋集合digestで固定し、全faultを重複なく一度だけ分類する。逆辺欠落は意味再降下、媒体親境界／stable root欠落は親要求とdeferred、誤責務3 edgeはsupersedeへ分類する | fault件数が同じなら内容入替を許すこと、分類外又は重複faultを残すこと、逆辺を機械追加して誤責務を温存すること、旧媒体BRを通常BRへ黙示昇格すること |
...
| G-REQ-AUTHORITY-CUTOVER | 要求baselineのapproved化とimplementation admissionを分離する。要求authorityはsemantic closure=0、active refinementのPO receipt付きfreeze、未決approval=0で切替可能だが、implementation_authorized=trueには別のPO receipt、独立Go review、同一commit/tree、manifest current inputとexact一致する新要求authority及びL2〜L6成果物集合を追加要求する。refinementのregistration／design-later自由文からstable obligation IDを導出し、登録receipt/evidence又はadmission対象design artifact clauseへの逆traceが全件揃うまで実装開始を拒否する | 要求freezeだけで設計済み・実装可能とみなすこと、provider/account/quota等の登録又はbackoff/session/rollback等の設計義務を落とすこと、架空artifact／任意文字列receipt／無関係な旧L2〜L6を組み合わせて実装許可すること |
...
`run_all.py --update-baseline` は通常の confirmed 文書について PO approval digest を必須とする。既存承認を新規承認へ読み替えない機械的な content-binding migration は、`G-MANIFEST-STATUS`／`G-CONFIRM-DIGEST` が検証した code-exact migration receipt（旧 PO digest・source commit/blob・現内容 digest・`grants_new_approval=false`）だけを baseline 更新の入力として認める。receipt が不正な場合は通常の承認欠落として拒否する。
