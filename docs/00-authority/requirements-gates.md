...
| G-REQ-DESIGN-NOT-STARTED | 要求baselineがrevisingの間、L2〜L6を`revalidation_required`かつ`implementation_input=false`に固定し、L2をdraftに保つ。README／CLAUDEの入口も旧baselineの再検証資料・現行未拘束境界を明示し、旧方式の現在形命令を禁止する | 旧画面・API・DDL・状態・AC/TC・実装単位、又はCLAUDEに残る旧方式制約を新VPS UI要求の設計済み成果物・現行実装命令として扱うこと |
