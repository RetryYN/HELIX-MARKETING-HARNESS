...
> pair: [strategy-loop-design_v0.1.md](strategy-loop-design_v0.1.md)／
> [strategy-loop-requirements_v0.1.md](../requirements/strategy-loop-requirements_v0.1.md)（要件・設計の両対）
> 機械可読正本: [json/strategy-tests.json](json/strategy-tests.json)（STC 台帳）
...
> 既存の TC 59／ITC 16／UTC 69 の分母は変更しない。
...
（既存 UTC 69 とは独立の追加テストであり、既存分母を変更しない）。
