<!-- GENERATED FILE — 編集禁止。正本は docs/design/json/cmp-contracts.json。再生成 = python3 scripts/render_views.py -->
...
- **要求 interface**: CMP-05 db.connect() が返す Connection（PRAGMA foreign_keys=ON 保証）／遷移表 docs/requirements/json/s0/transitions.json（パッケージデータ同梱）／CMP-02 validate_strategic_brief（lower run start ガードの brief 検証実体）／Clock 注入（現在時刻の直接取得禁止）
...
- **提供 interface**: issue_task(conn, loop_run_id, step, clock) -> int — 決定的 idempotency_key（loop_run_id:step:attempt）でのタスク発行・冪等再利用／claim(conn, task_id, execution_id, clock) -> None — row_version 楽観ロックによる lease 取得（author 資格検査つき）／run_microloop(conn, task_id, executor, verifier, retry_limit_key) -> MicroloopResult — submit→verify 反復、retry_limit 到達で escalate／resume(conn, entity_type, entity_id, clock) -> ResumeAction — s0-contract §3.3 再開規則の実装（判断根拠は DB 行のみ）／assign(conn, author_role, verifier_role) -> Assignment — principal の異なる active agent 組の割当（自己審査封じ）／load(conn, workflow_key, version) -> WorkflowDef ／ run_step(conn, task_id, step, ctx) -> StepOutcome — WF 定義の schema 検証つき読込と実行／issue_strategic_brief(conn, brief, clock) -> int — 正準化 JSON の SHA-256 digest 決定計算つき brief 版発行（SCM-02）／supersede_strategic_brief(conn, old_brief_id, new_draft, clock) -> int — 新版 INSERT＋旧版 superseded を単一 transaction（SCM-02）／validate_strategic_brief(conn, brief_id, held_digest, clock) -> ValidBrief — active・digest 一致・期間内の検証（SCM-03 の実体）／generate_tactical_learning_packet(conn, loop_run_id, packet, clock) -> int — 下位終端遷移と同一 transaction での TLP 生成（SCM-04）／get_tactical_learning_packet(conn, loop_run_id) -> TlpRecord | None — 上流が読む唯一の還流読取り口
- **要求 interface**: CMP-01 transition()（すべての状態変更はここ経由）／CMP-03 ゲート API（require_pair／check_complete 等 — guard・公開前検査）／CMP-04 evidence.record()（ステップ出力の証跡化）／CMP-05 db.connect() ／ CMP-06 config.get()（retry_limit・lease_ttl_sec 等）／CMP-08〜13 コネクタ・制作・計測 API（呼ぶだけ — 逆依存なし）／Clock/Rng 注入
- **責務境界**: やる: タスク発行・割当・lease・マイクロループ・WF ステップ実行の進行制御、再開規則、brief 発行/検証/supersede と TLP 生成（戦略層 S0 拡張）。やらない: 拒否判定の実体（CMP-01/03 に集約）、外部 I/O の実装（コネクタに委譲）、証跡の直接 INSERT（CMP-04 経由）、遷移の直接 UPDATE（CMP-01 経由）。
- **依存方向**: ドメイン層（kernel）。cli から呼ばれ、CMP-01/03/04/05/06 に依存し、connectors/content/measure を呼ぶだけ（単方向 cli→kernel→gates→基盤）。
- **データフロー**: ループステップ到達 → WF 定義から tasks 行生成（冪等）→ 割当・claim → ステップ実行（コネクタ呼出しは operation_log 証跡化→状態遷移の順）→ 出力を evidence 経由保存 → 遷移は CMP-01 へ。戦略層: brief draft → schema 検証 → digest 計算 → INSERT／下位終端 → TLP 組立 → 終端遷移と同一 transaction で INSERT。
- **状態所有者**: tasks の発行・lease 列（lease_owner_execution_id/lease_expires_at/heartbeat_at）、strategic_briefs／tactical_learning_packets への書込み（issue/supersede/generate の 3 API に閉域） ／ **transaction 所有者**: タスク採番・発行（単一 transaction）、brief supersede（新版＋旧版更新）、TLP 生成（下位終端遷移と同一 transaction — CMP-01 と共有）。状態遷移そのものの transaction は CMP-01
...
- **degradation／復旧**: クラッシュ後は resume() が DB 行のみから再開（pending 再 claim／sent はリモート照合→confirmed 化、照合不能は escalate・再送禁止／verifying は既存 PASS/FAIL 再採用で retry 二重加算なし）。issue_task は同一 (loop_run_id, step_key) の非終端 task 再利用で冪等。
...
- **trace**: FN = FN-102 FN-103 FN-104 FN-105 ／ DU = DU-02 DU-03 DU-04 ／ 独立設計書 = error-taxonomy_v0.1.md
...
- **責務境界**: やる: evidence の型契約検証（kind 別必須キー・列整合・PASS 値・reviewer≠author・桁数・approvals 相互整合）、INSERT、secret 混入拒否、read-only 参照。やらない: 証跡内容の生成（呼出し側）、UPDATE/DELETE（API 自体が存在しない）、状態遷移、外部 I/O。
- **依存方向**: 基盤層（evidence）。kernel・gates・コネクタのストア副層から呼ばれ、db にのみ依存。ドメイン層を import しない。
- **データフロー**: 証跡候補（task_id/kind/value/payload/整合列）→ kind 型契約・混入検査 → 合格のみ INSERT → evidence_id 返却。違反は GateRejected で DB 不変。
...
- **trace**: FN = FN-305 ／ DU = DU-12 ／ 独立設計書 = brand-isolation-design_v0.1.md、error-taxonomy_v0.1.md
...
- **提供 interface**: launch(headed, storage_state_path=None) -> BrowserSession — Playwright 起動・storage_state 保存/再利用・headed/headless 切替（WSLg）／screenshot(session, url, out_path) -> Path — URL 到達確認つき capture（file_hash 固定は呼出し側）
- **要求 interface**: Playwright ランタイム／CMP-07 get_credential()／check_endpoint()（ログイン credential の供給と接続前検査）／storage_state 永続ファイル（セッション再利用）
- **責務境界**: やる: ブラウザ起動・セッション永続・スクリーンショット取得の機構提供。やらない: セレクタ・操作手順の保持（CMP-09 攻略地図から供給）、業務状態・証跡の直接書込み（呼出し側が kernel/evidence API 経由）、書込み系の外部操作（S0 では GA4 フォールバック閲覧とスクショのみ）。
- **依存方向**: コネクタ層（connectors）。kernel（CMP-02）・計測（CMP-13）から呼ばれ、CMP-07（credential）と Playwright にのみ依存。業務テーブルへ直接触れない。
- **データフロー**: 起動要求 → Playwright 起動＋storage_state 復元 → セッション返却 → 操作（呼出し側が playbook 手順で駆動）→ スクショ・取得物をファイルへ → 証跡化は呼出し側。
- **状態所有者**: なし（stateless — storage_state ファイルは機構上の永続だが業務状態ではない） ／ **transaction 所有者**: なし（DB transaction に参加しない）
...
- **提供 interface**: get(conn, service, operation, route_type) -> Playbook — 有効な手順書の取得（broken は返さない）／record_success(conn, playbook_id, agent_id, clock) -> None — last_success_at 更新（AC-42）／record_failure(conn, playbook_id, clock) -> None — consecutive_failures 加算・閾値到達で broken 降格
- **要求 interface**: CMP-05 db.connect() が返す Connection（playbooks テーブル）／CMP-06 config.get()（連続失敗閾値）／Clock 注入（last_success_at/last_failure_at）
- **責務境界**: やる: ブラウザ操作手順書（playbooks）の保存・参照・成功/失敗記録・broken 降格。永続化はストア副層 playbooks_store に限定（生 SQL はここだけ — 基本設計 §1 規約 3）。やらない: 手順の実行（CMP-08＋呼出し側）、業務状態（loop_runs/tasks/evidence）への書込み、手順の自動修復。
- **依存方向**: コネクタ層の所掌ストア副層。kernel・計測から呼ばれ、db/config にのみ依存。外部 I/O コードと分離。
- **データフロー**: 手順要求（service/operation/route_type）→ 有効 playbook 行返却 → 実行結果 → record_success/failure で鮮度・健全性を更新 → 連続失敗で broken 降格。
- **状態所有者**: playbooks テーブル（手順書・鮮度・健全性状態。コネクタ所掌テーブルのストア副層所有） ／ **transaction 所有者**: playbooks 行の単発更新（自トランザクション。業務遷移 transaction には参加しない）
- **エラー分類**: PlaybookMissing: 該当手順書なし → FatalError（手順なしで操作を試みない — fail-close）／PlaybookBroken: broken 状態の手順書要求 → FatalError・escalate（人間による手順更新待ち）／IntegrityError: FK・UNIQUE 違反 → GateRejected 正規化
...
- **要求 interface**: CMP-03 PairPass（require_pair/establish のみが生成 — 公開系の必須引数）／CMP-07 get_credential()（Application Password）／check_endpoint()／CMP-04 evidence.record()（operation_log／published_url 証跡の派生記録）／CMP-06 config.get()（Docker WP base URL allow-list・レート間隔）／external_operations テーブル（prepared→sent→confirmed の操作状態機械）
- **責務境界**: やる: WP REST（下書き・公開・メディア）の外部書込み実行、idempotency key 事前照合・再送抑止、external_operations の操作状態管理、公開後の assets 登録。低レベル client は `_client` 非公開でゲート未通過経路を構造的に封鎖。やらない: 公開可否の判定（CMP-03 の PairPass が前提）、業務状態遷移（kernel 経由）、Docker WP 以外への書込み（allow-list 外は接続前拒否）。
- **依存方向**: コネクタ層（connectors）。kernel（CMP-02 の WF 実行）からのみ呼ばれ、CMP-03（型として）・04・06・07 に依存。業務状態を直接書かない（assets はストア副層 `_assets_store` 所掌）。
- **データフロー**: PairPass＋HTML＋idempotency_key → 同 key の external_operations 照合（confirmed 済みは再送せず補完）→ prepared→sent→confirmed 各コミット → WP 側 meta に key 保存 → assets INSERT → published_url 証跡（asset_id 成立後）→ 遷移は kernel へ。
- **状態所有者**: external_operations 行（自操作分）と assets 行（ストア副層 `_assets_store` 経由）。loop_runs/tasks は書かない ／ **transaction 所有者**: external_operations の各状態コミット（prepared/sent/confirmed を各々コミット — 送信直後クラッシュの検出窓）と assets INSERT。外部操作は業務遷移 transaction の外（operation_log 証跡化 → 状態遷移の順）
...
- **要求 interface**: transport interface（本番: Claude Code アプリ通知、テスト: mock fixture — 差替可能）／CMP-04 evidence.record()（approval 証跡）／CMP-01 transition()（waiting／failed／escalated 遷移は kernel 経由）／CMP-06 config.get()（approval_retry_limit・有効期限）／approvals テーブル（ストア副層 approvals_store 経由）
- **責務境界**: やる: binding 3 項目束縛の承認要求・応答受領・approvals＋approval 証跡の相互整合記録、binding 不一致応答の無効化。やらない: 承認判断そのもの（人間）、承認対象操作の実行（CMP-10 等）、状態遷移の直接実行（DU-01 経由 — pending 中は親 loop_run を waiting にし task は進行させない）。
- **依存方向**: コネクタ層（connectors）。kernel から呼ばれ、transport・evidence・config に依存。業務状態は kernel 経由でのみ動かす。
- **データフロー**: 承認要求（binding）→ transport 通知＋approvals 行 INSERT → 人間応答 → binding 全項目照合（1 項目でも不一致は無効）→ approved: 証跡 INSERT＋evidence_id 更新（単一 transaction）／rejected: non_retryable_failure で task failed／expired: 再要求・approval_retry_limit 到達で escalated。
- **状態所有者**: approvals テーブル（ストア副層 approvals_store 所有）。loop_runs/tasks の状態は所有しない（kernel 経由） ／ **transaction 所有者**: `_record_decision` の approval 証跡 INSERT＋approvals.evidence_id 更新（単一 transaction — 相互整合の中間状態を外部に見せない）。遷移 transaction は CMP-01
...
- **要求 interface**: CMP-03 check_metric_type()（有料指標の登録拒否 — DB CHECK と二重）／CMP-07 resolve()／get_credential()（GA4 経路解決・API credential）／CMP-08 launch()／screenshot()（ブラウザフォールバックとスクショ証跡）／CMP-04 evidence.record()（screenshot・operation_log・取得 hash 証跡）／Clock 注入（imported_at）
- **責務境界**: やる: KPI ツリー管理、GA4 データの read-only 取得・hash 固定・パース・隔離つき投入。やらない: 有料指標の受理（CMP-03 で拒否）、書込み系 GA4 操作（組み立て時点で拒否 — read-only 保証）、壊れ行の黙殺（隔離ファイル＋件数証跡化 — AC-62）、戦略判断（KPI は観測のみで戦略正本ではない — SR-12）。
- **依存方向**: 計測層（measure）。kernel（WF-MEAS-1）から呼ばれ、gates・registry・connectors・evidence・db に依存。業務状態遷移は kernel 経由。
- **データフロー**: 経路解決 → GA4 API 取得（阻害時のみブラウザエクスポート）→ 即 SHA-256 固定＋スクショ・operation_log 証跡 → parse で schema/type 検証・壊れ行隔離 → ingest 前に raw hash 再計算照合 → 正常行のみ単一 transaction 投入（途中失敗は全 rollback）。
