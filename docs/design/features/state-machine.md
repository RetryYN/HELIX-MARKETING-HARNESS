...
> 正準参照: 遷移表の正準は [json/s0/transitions.json](../../requirements/json/s0/transitions.json) ＋
> [s0-contract_v0.1.md §3](../../requirements/s0-contract_v0.1.md)。状態図・ガード 6 分類・発火権限・
> 競合制御の設計層は [state-machine-design_v0.1.md](../state-machine-design_v0.1.md)（本書の上位 —
> 本書は同書 §2〜§4 を実装レベルへ降下する）。API 署名の正本は
> [detailed-design_v0.1.md DU-01](../detailed-design_v0.1.md)。遷移表・状態図は再掲しない。
...
- **invariant**: `loop_runs.state`・`tasks.state` の UPDATE 文は本モジュール以外に存在しない
  （[db-design_v0.1.md §2](../db-design_v0.1.md) 横断規則）。終端状態からの遷移要求は常に拒否。
...
1. イベントごとに `[G1 権限, G2 構造, G3 brief, G4 リトライ境界, G5 証跡完備]` から該当分類の
   ...
   [state-machine-design_v0.1.md §2](../state-machine-design_v0.1.md)）。
...
`TransitionRejected` は `GateRejected` の kernel 具体型（[error-taxonomy_v0.1.md §3.1](../error-taxonomy_v0.1.md)）
