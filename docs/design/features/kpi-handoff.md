...
> SR-12（[sr-contracts.json](../../requirements/json/strategy/sr-contracts.json) — 観測背骨と戦略正本の分離）。
> スキーマ = [s0-contract_v0.1.md §2](../../requirements/s0-contract_v0.1.md)
> （`pair_kpi_measure`・`kpi_nodes`・`measurements`・`learnings`・`tactical_learning_packets` — DDL 再掲禁止）。
> 上位設計: [basic-design_v0.1.md](../basic-design_v0.1.md)（CMP-03/13）／
> [strategy-loop-design_v0.1.md](../strategy-loop-design_v0.1.md)（TLP 経路）
...
2. **TLP metrics（KPI ノード参照）**: 下位 run 終端時の TLP 生成（DU-02）は、`metrics_json` の
   ...
   [tactical-learning-packet.schema.json](../../requirements/json/strategy/tactical-learning-packet.schema.json)）。
