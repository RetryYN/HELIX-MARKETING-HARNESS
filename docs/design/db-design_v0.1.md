...
> [s0-contract_v0.1.md §2](../requirements/s0-contract_v0.1.md)（JSON 正本 =
> [json/s0/ddl.sql](../requirements/json/s0/ddl.sql)）。migration 規則の正準は同 §5、
> 接続規約の正準は同 §1。本書は DDL を再掲しない — 契約と矛盾したら契約を優先し本書を改訂する。
> 上位設計: [basic-design_v0.1.md](basic-design_v0.1.md)（CMP 台帳・層分離）／
> [detailed-design_v0.1.md](detailed-design_v0.1.md)（DU-10/DU-11）
...
## 3. append-only 群・可変群とトリガ 11 本の意図
...
### 3.2 トリガ 11 本の意図（本文は s0-contract §2 が正準）
...
6. **0001 = 正準 DDL と等価**: G-DDL-APPLY／G-DDL-SYNC が JSON 正本
   （[json/s0/ddl.sql](../requirements/json/s0/ddl.sql)）との等価性を機械検査する。手書き同期に依存しない。
...
- 遷移 tx は `BEGIN IMMEDIATE` で開始し、書込みロックを先取して guard 評価中のロスト
  ...
  [state-machine-design_v0.1.md §5](state-machine-design_v0.1.md)）。
