...
def test_ddl_applies_to_empty_db_with_25_tables_37_triggers(conn: sqlite3.Connection) -> None:
    assert conn.execute("SELECT count(*) FROM sqlite_master WHERE type='table'").fetchone()[0] == 25
    assert conn.execute("SELECT count(*) FROM sqlite_master WHERE type='trigger'").fetchone()[0] == 37
...
def test_apply_all_empty_db_creates_25_tables_and_37_triggers() -> None:
