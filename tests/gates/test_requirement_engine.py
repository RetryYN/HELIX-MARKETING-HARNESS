...
def test_candidate_ir_v2_matches_helix_harness_shape_without_authority_promotion() -> None:
    ...
    assert len(built["shards"]["requirements"]) == 47
    assert len(built["shards"]["system_contracts"]) == 47
    assert len(built["shards"]["acceptance_cases"]) == 141
    assert len(built["shards"]["system_tests"]) == 47
    assert len(built["shards"]["refinement_contracts"]) == 47
    assert all(
